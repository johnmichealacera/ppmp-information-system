...
export async function POST(
  ...
  try {
    ...
    if (ppmp.status !== 'SUBMITTED') {
      return NextResponse.json(
        { error: 'Only submitted PPMPs can be rejected' },
        { status: 400 }
      );
    }
    ...
    // Update PPMP status to rejected
    ...
    // Send notification to PPMP preparer
    await db.notification.create({
      data: {
        type: 'ppmp_rejected',
        title: 'PPMP Rejected',
        message: `Your PPMP "${updatedPpmp.title}" has been rejected. Remarks: ${remarks}`,
    ...
    console.error('Error rejecting PPMP:', error);
    return NextResponse.json(
